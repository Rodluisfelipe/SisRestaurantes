...
docker stop sisrestaurantes-backend 2>/dev/null
docker rm sisrestaurantes-backend 2>/dev/null

# Start new container with test Wompi keys
docker run -d \
  ...
  -e JWT_REFRESH_SECRET='jwt_refresh_secret_super_seguro_para_produccion_2024_sisrestaurantes_pipe95141007' \
