...
const BusinessClosedModal = ({ isOpen, onClose, businessStatus }) => {
  ...
  const getNextOpenInfo = () => {
    if (!businessStatus?.nextOpenTime) return null;
    ...
    return {
      day: dayNames[businessStatus.nextOpenTime.day] || businessStatus.nextOpenTime.day,
      time: businessStatus.nextOpenTime.time
    };
  };

  const nextOpen = getNextOpenInfo();
      ...
      >
        ...
        >
          ...
          <div className="p-5 space-y-3">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-600">Abrimos</p>
                    <p className="text-sm font-bold text-gray-900 truncate">{nextOpen.day} - {nextOpen.time}</p>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {businessConfig?.socialMedia && (businessConfig.socialMedia.facebook?.isVisible || businessConfig.socialMedia.instagram?.isVisible || businessConfig.socialMedia.tiktok?.isVisible) && (
              <div className="flex justify-center space-x-3 py-2">
                ...
                {businessConfig.socialMedia.instagram?.isVisible && businessConfig.socialMedia.instagram?.url && (
                  <motion.a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 rounded-full flex items-center justify-center text-white shadow-md"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                {businessConfig.socialMedia.tiktok?.isVisible && businessConfig.socialMedia.tiktok?.url && (
                  <motion.a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 bg-black rounded-full flex items-center justify-center text-white shadow-md"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={onClose}
              className="w-full px-5 py-2.5 font-semibold rounded-xl transition-all duration-200 shadow-md text-sm"
              style={{
