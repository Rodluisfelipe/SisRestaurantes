        ...
        <div className="flex flex-col items-center px-4">
          ...
          {reviewStats && reviewStats.totalReviews > 0 && (
            <button
              ...
              className={`inline-flex items-center gap-1 mt-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium transition-all active:scale-95 ${
                businessConfig.coverImage
                  ? 'bg-white/20 backdrop-blur-sm text-white hover:bg-white/30'
                  : 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100'
              }`}
            >
              <svg className="w-3 h-3" viewBox="0 0 24 24" fill="#facc15">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
              </svg>
              <span>{reviewStats.averageRating.toFixed(1)}</span>
              <span className={businessConfig.coverImage ? 'text-white/70' : 'text-yellow-600'}>·</span>
              <span className={businessConfig.coverImage ? 'text-white/70' : 'text-yellow-600'}>
                {reviewStats.totalReviews} reseña{reviewStats.totalReviews !== 1 ? 's' : ''}
              </span>
              <svg className="w-2.5 h-2.5 ml-0.5 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3">
