...
export default function CrewWorkerProfileModal({ workerId, businessId, matchScore, onClose, onAccept, onReject, onChat, canChat }) {
        ...
        >
          ...
          {worker && (
            <>
              ...
              {(onAccept || onReject || (canChat && onChat)) && (
                <div className="border-t border-slate-200 px-5 py-3 bg-white flex gap-2">
                  ...
                  {canChat && onChat && !onAccept && (
                    <button
                      onClick={onChat}
                      className="flex-1 inline-flex items-center justify-center gap-2 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-[13px] shadow-md shadow-slate-900/20 transition"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                      Enviar mensaje
                    </button>
                  )}
                </div>
