...
const ReviewsSheet = ({ show, onClose, businessId, reviewStats, theme }) => {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-[180] transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`}
        style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}
        onClick={handleClose}
      />
      {/* Sheet */}
      <div
        className={`fixed inset-x-0 bottom-0 z-[181] bg-white rounded-t-2xl max-h-[85vh] flex flex-col shadow-2xl transition-transform duration-300 ${visible ? 'translate-y-0' : 'translate-y-full'}`}
      >
        {/* Handle */}
