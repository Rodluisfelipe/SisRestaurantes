...
function DiffView({ before, after, action }) {
  if (action === 'delete' && before) {
    return (
      <div>
        <p className="text-xs text-red-400 font-medium mb-2">Datos eliminados:</p>
        <pre className="text-xs text-white/60 bg-black/30 rounded-lg p-3 overflow-auto max-h-72 whitespace-pre-wrap break-words">
  ...
  if (action === 'create' && after) {
    return (
      <div>
        <p className="text-xs text-emerald-400 font-medium mb-2">Datos creados:</p>
        <pre className="text-xs text-white/60 bg-black/30 rounded-lg p-3 overflow-auto max-h-72 whitespace-pre-wrap break-words">
  ...
  return <p className="text-xs text-white/40">Sin datos de snapshot disponibles</p>;
}
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <p className="text-white/40 text-xs uppercase tracking-wider">Total Logs</p>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {stats.byAction?.map(a => (
                <span key={a._id} className={`text-xs px-2 py-0.5 rounded-full ${ACTION_LABELS[a._id]?.color || 'bg-white/10 text-white/60'}`}>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <p className="text-white/40 text-xs uppercase tracking-wider">Por Recurso</p>
            <div className="flex flex-wrap gap-1.5 mt-2">
              {stats.byResource?.map(r => (
                <span key={r._id} className="text-xs px-2 py-0.5 rounded-full bg-white/10 text-white/50">
      ...
      <div className="flex items-center justify-between">
        <p className="text-white/40 text-sm">{total} registro{total !== 1 ? 's' : ''} encontrado{total !== 1 ? 's' : ''}</p>
        {totalPages > 1 && (
          <p className="text-white/30 text-xs">Página {page} de {totalPages}</p>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <svg className="w-12 h-12 text-white/20 mx-auto mb-3" fill="none" stroke="currentColor" strokeWidth={1} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
          </svg>
          <p className="text-white/30 text-sm">No hay registros de auditoría</p>
          {hasFilters && <p className="text-white/20 text-xs mt-1">Intenta cambiar los filtros</p>}
        </div>
      ) : (
        <div className="space-y-2">
          {logs.map(log => {
            const actionInfo = ACTION_LABELS[log.action] || { label: log.action, color: 'bg-white/10 text-white/50', icon: '?' };
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Time */}
                  <span className="text-xs text-white/25 shrink-0 w-20 text-right">
                    {timeAgo(log.createdAt)}
                  </span>

                  {/* Chevron */}
                  <svg className={`w-4 h-4 text-white/30 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      ...
                      <div className="px-4 pb-4 border-t border-white/5 pt-3 space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                          <div>
                            ...
                            <span className="text-white/60 ml-1">{formatDate(log.createdAt)}</span>
                          </div>
                          <div>
                            <span className="text-white/30">Usuario:</span>
                            <span className="text-white/60 ml-1">{log.userEmail || '—'}</span>
                          </div>
                          <div>
                            <span className="text-white/30">Rol:</span>
                            <span className="text-white/60 ml-1">{log.userRole || '—'}</span>
                          </div>
                          <div>
                            <span className="text-white/30">IP:</span>
                            <span className="text-white/60 ml-1 font-mono">{log.ip || '—'}</span>
                          </div>
                          <div className="col-span-2">
                            <span className="text-white/30">Resource ID:</span>
                            <span className="text-white/60 ml-1 font-mono text-[11px]">{log.resourceId}</span>
                          </div>
                          <div className="col-span-2">
                            <span className="text-white/30">Negocio:</span>
                            <span className="text-white/60 ml-1">{log.businessName || '—'}</span>
                          </div>
                          {log.reverted && (
                            <>
                              <div>
                                <span className="text-white/30">Revertido por:</span>
                                <span className="text-yellow-400/70 ml-1">{log.revertedBy}</span>
                              </div>
                              <div>
                                <span className="text-white/30">Fecha revert:</span>
                          ...
                          <div className="flex justify-end pt-2">
                            {revertConfirm === log._id ? (
                              <div className="flex items-center gap-2">
                                <span className="text-xs text-amber-400">Confirmar revert?</span>
                                <button
                                  onClick={() => handleRevert(log._id)}
                                  disabled={reverting === log._id}
                                  className="text-xs bg-red-500/20 text-red-400 hover:bg-red-500/30 px-3 py-1.5 rounded-lg transition disabled:opacity-50"
                                >
                                  {reverting === log._id ? 'Revirtiendo...' : 'Sí, revertir'}
                                </button>
                                <button
                                  onClick={() => setRevertConfirm(null)}
                                  className="text-xs text-white/40 hover:text-white/60 px-2 py-1.5 transition"
                                >
                                  Cancelar
                                </button>
                              </div>
                              ...
                              <button
                                ...
                                className="text-xs bg-white/5 text-white/50 hover:bg-white/10 hover:text-white/70 px-3 py-1.5 rounded-lg transition flex items-center gap-1.5"
                              >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <button
            ...
            disabled={page <= 1}
            className="px-3 py-1.5 text-xs text-white/50 bg-white/5 rounded-lg hover:bg-white/10 transition disabled:opacity-30 disabled:cursor-not-allowed"
          >
            Anterior
          </button>
