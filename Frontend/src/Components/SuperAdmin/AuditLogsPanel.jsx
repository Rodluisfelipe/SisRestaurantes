...
const ACTION_LABELS = {
  create: { label: 'Creado', color: 'bg-emerald-200 text-emerald-600', icon: '+' },
  update: { label: 'Editado', color: 'bg-cyan-200 text-cyan-600', icon: '✎' },
  delete: { label: 'Eliminado', color: 'bg-red-200 text-red-600', icon: '×' },
  toggle: { label: 'Toggle', color: 'bg-amber-200 text-amber-600', icon: '⇄' },
  reorder: { label: 'Reordenado', color: 'bg-purple-200 text-purple-600', icon: '↕' },
};
...
function DiffView({ before, after, action }) {
  if (action === 'delete' && before) {
    return (
      <div>
        <p className="text-xs text-red-600 font-medium mb-2">Datos eliminados:</p>
        <pre className="text-xs text-slate-600 bg-black/30 rounded-lg p-3 overflow-auto max-h-72 whitespace-pre-wrap break-words">
  ...
  if (action === 'create' && after) {
    return (
      <div>
        <p className="text-xs text-emerald-600 font-medium mb-2">Datos creados:</p>
        <pre className="text-xs text-slate-600 bg-black/30 rounded-lg p-3 overflow-auto max-h-72 whitespace-pre-wrap break-words">
  ...
  if ((action === 'update' || action === 'toggle') && before && after) {
    ...
    if (changes.length === 0) return <p className="text-xs text-slate-500">Sin cambios detectados</p>;
    return (
      <div className="space-y-2">
        <p className="text-xs text-cyan-600 font-medium mb-2">Campos modificados ({changes.length}):</p>
        {changes.map(key => (
  ...
  return <p className="text-xs text-slate-500">Sin datos de snapshot disponibles</p>;
}
...
export default function AuditLogsPanel() {
 ...
 feedback.type === 'success' ? 'bg-emerald-500/90 text-slate-900' : 'bg-red-500/90 text-slate-900'
 }`}
      ...
      <div className="bg-slate-100 border border-slate-200 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-cyan-500 [&>option]:bg-white"
          >
          ...
          <input
            ...
            className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-cyan-500 [color-scheme:dark]"
            placeholder="Desde"
          />
          <input
            type="date"
      ...
      <div className="flex items-center justify-between">
        <p className="text-slate-500 text-sm">{total} registro{total !== 1 ? 's' : ''} encontrado{total !== 1 ? 's' : ''}</p>
        {totalPages > 1 && (
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <svg className="w-12 h-12 text-slate-400 mx-auto mb-3" fill="none" stroke="currentColor" strokeWidth={1} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
          </svg>
          <p className="text-slate-500 text-sm">No hay registros de auditoría</p>
          {hasFilters && <p className="text-slate-400 text-xs mt-1">Intenta cambiar los filtros</p>}
        </div>
      ) : (
        <div className="space-y-2">
          {logs.map(log => {
 ...
 log.reverted ? 'border-yellow-500/30 opacity-60' : 'border-slate-200 hover:border-slate-300'
              ...
              >
                ...
                >
                  ...
                  {/* Chevron */}
                  <svg className={`w-4 h-4 text-slate-500 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      ...
                      <div className="px-4 pb-4 border-t border-slate-200 pt-3 space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                          <div>
                            ...
                            <span className="text-slate-600 ml-1">{formatDate(log.createdAt)}</span>
                          </div>
                          <div>
                            <span className="text-slate-500">Usuario:</span>
                            <span className="text-slate-600 ml-1">{log.userEmail || '—'}</span>
                          </div>
                          <div>
                            <span className="text-slate-500">Rol:</span>
                            <span className="text-slate-600 ml-1">{log.userRole || '—'}</span>
                          </div>
                          <div>
                            <span className="text-slate-500">IP:</span>
                            <span className="text-slate-600 ml-1 font-mono">{log.ip || '—'}</span>
                          </div>
                          <div className="col-span-2">
                            <span className="text-slate-500">Resource ID:</span>
                            <span className="text-slate-600 ml-1 font-mono text-[11px]">{log.resourceId}</span>
                          </div>
                          <div className="col-span-2">
                            <span className="text-slate-500">Negocio:</span>
                            <span className="text-slate-600 ml-1">{log.businessName || '—'}</span>
                          </div>
                          {log.reverted && (
                            <>
                              <div>
                          ...
                          <div className="flex justify-end pt-2">
                            {revertConfirm === log._id ? (
                              <div className="flex items-center gap-2">
                                <span className="text-xs text-amber-600">Confirmar revert?</span>
                                <button
                                  onClick={() => handleRevert(log._id)}
                                  disabled={reverting === log._id}
                                  className="text-xs bg-red-200 text-red-600 hover:bg-red-200 px-3 py-1.5 rounded-lg transition disabled:opacity-50"
                                >
                                  {reverting === log._id ? 'Revirtiendo...' : 'Sí, revertir'}
                                </button>
                                <button
                                  onClick={() => setRevertConfirm(null)}
                                  className="text-xs text-slate-500 hover:text-slate-600 px-2 py-1.5 transition"
                                >
                                  Cancelar
                                </button>
                              </div>
