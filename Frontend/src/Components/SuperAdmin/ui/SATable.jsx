...
export default function SATable({ columns, data = [], onRowClick, rowKey = '_id', loading, stagger = true }) {
  if (loading) {
    return (
      <div className="space-y-2">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-14 bg-white/[0.02] border border-white/[0.04] rounded-lg animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      {/* Desktop header */}
      <div className="hidden md:grid items-center gap-3 px-4 py-2 text-[11px] font-medium text-white/30 uppercase tracking-wider" 
      ...
      <div className="space-y-1">
        {data.map((row, i) => (
          <motion.div
            ...
            className={`grid items-center gap-3 px-4 py-3 rounded-lg border border-white/[0.04] bg-white/[0.015]
          ...
          >
            {columns.map(col => (
              <div key={col.key} className={`text-sm ${col.hideOnMobile ? 'hidden md:block' : ''} ${col.className || ''}`}>
                {col.render ? col.render(row) : <span className="text-white/70 truncate">{row[col.key]}</span>}
              </div>
            ))}
          </motion.div>
        ))}
