...
export default function SACard({ children, className = '', hover = false, padding = 'p-5', onClick, as = 'div' }) {
  const base = `bg-white/[0.02] border border-white/[0.06] rounded-xl ${padding} ${
    hover ? 'hover:bg-white/[0.04] hover:border-white/[0.1] transition-all duration-200 cursor-pointer' : ''
  } ${className}`;
