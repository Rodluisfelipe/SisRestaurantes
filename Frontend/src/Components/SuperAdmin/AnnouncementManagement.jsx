    ...
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="bg-red-500/20 border border-red-500/50 text-red-200 px-4 py-3 rounded-xl flex items-center justify-between"
          >
            <span>{error}</span>
        ...
        {showForm && (
          ...
          >
            <form onSubmit={handleSubmit} className="bg-[#2A3544] rounded-2xl p-5 sm:p-6 border border-[#333F50] space-y-5">
              ...
              {/* Title */}
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1.5">Título *</label>
                <input
                  type="text"
                  value={form.title}
                  onChange={e => setForm(prev => ({ ...prev, title: e.target.value }))}
                  className="w-full bg-[#1A2433] border border-[#333F50] text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5FF9B4] focus:border-transparent outline-none transition-all text-sm"
              ...
              {/* Body */}
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1.5">Contenido *</label>
                <textarea
                  value={form.body}
                  onChange={e => setForm(prev => ({ ...prev, body: e.target.value }))}
                  className="w-full bg-[#1A2433] border border-[#333F50] text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5FF9B4] focus:border-transparent outline-none transition-all text-sm min-h-[120px] resize-y"
                  placeholder="Escribe el contenido del anuncio..."
                  maxLength={2000}
                  required
                />
                <p className="text-white/40 text-xs mt-1">{form.body.length}/2000</p>
              </div>

              {/* Priority */}
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1.5">Prioridad</label>
                    ...
                    <button
                      ...
                      className={`px-3 py-2.5 rounded-xl text-sm font-medium transition-all border-2 flex items-center justify-center gap-1.5 ${
                        form.priority === opt.value
                          ...
                          : 'bg-[#1A2433] text-white/70 border-[#333F50] hover:border-white/30'
                      }`}
                    >
                      <span>{opt.icon}</span>
                      <span>{opt.label}</span>
              ...
              {/* Image */}
              <div>
                <label className="block text-white/80 text-sm font-medium mb-1.5">Imagen (opcional)</label>
                <div className="flex items-start gap-4">
                  <label className="flex-1 cursor-pointer">
                    <div className="bg-[#1A2433] border-2 border-dashed border-[#333F50] rounded-xl p-4 text-center hover:border-[#5FF9B4]/50 transition-colors">
                      <svg className="w-8 h-8 mx-auto text-white/40 mb-2" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z" />
                      </svg>
                      <p className="text-white/50 text-xs">Clic para subir imagen</p>
                      <p className="text-white/30 text-xs mt-1">JPG, PNG, WEBP (máx 5MB)</p>
                    </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="button"
                  onClick={resetForm}
                  className="px-6 py-3 bg-[#333F50] text-white rounded-xl hover:bg-[#333F50]/80 transition-colors text-sm font-medium"
                >
                  Cancelar
                </button>
              </div>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-white/50 text-sm">Cargando anuncios...</p>
        </div>
      ) : announcements.length === 0 ? (
        <div className="text-center py-16">
          <span className="text-5xl mb-4 block">📭</span>
          <p className="text-white/60 text-lg font-medium">No hay anuncios creados</p>
          <p className="text-white/40 text-sm mt-1">Crea tu primer anuncio para notificar a los negocios</p>
        </div>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${pConfig.color} text-white`}>
                      ...
                      <h4 className="text-white font-bold text-base truncate">{ann.title}</h4>
                      <p className="text-white/50 text-sm mt-1 line-clamp-2">{ann.body}</p>
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      ...
                      >
                        <svg className="w-4 h-4 text-white/40 group-hover:text-[#5FF9B4] transition-colors" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                          <path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                        </svg>
                        <span className="text-white/50 group-hover:text-[#5FF9B4] transition-colors font-medium">
            ...
            >
              <div className="p-5 sm:p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button
                    onClick={() => setShowDetail(false)}
                    className="w-8 h-8 rounded-full bg-[#333F50] text-white/60 hover:text-white flex items-center justify-center transition-colors"
                  >
                    ✕
                  </button>
                </div>
                ...
                {/* Body */}
                <p className="text-white/70 text-sm whitespace-pre-line mb-6">{selectedAnnouncement.body}</p>

                {/* Seen Stats */}
                <div className="bg-[#2A3544] rounded-xl p-4 border border-[#333F50]">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-white font-bold text-sm">Lectura por negocios</h4>
