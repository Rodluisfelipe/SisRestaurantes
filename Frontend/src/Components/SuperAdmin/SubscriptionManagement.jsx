  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Suscripciones</h2>
          <p className="text-xs text-slate-500 mt-0.5">Administra los planes de suscripción</p>
        </div>
        <SAButton
          variant="primary"
          size="md"
      ...
      {subscriptions.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-slate-500 mb-1">No hay suscripciones</p>
          <p className="text-xs text-slate-400 mb-4">Crea la primera suscripción para comenzar</p>
          ...
          {subscriptions.map((subscription, index) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white border border-slate-200 rounded-xl p-4 hover:bg-slate-100 transition-colors"
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        <span className="text-xs text-slate-500">{getPlanText(commercialPlan)} - {getBillingCycleText(billingCycle)}</span>
                        <span className="text-xs font-medium text-slate-700 tabular-nums">${subscription.price?.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-[11px] text-slate-500 mt-1">Hasta {new Date(subscription.endDate).toLocaleDateString('es-ES')}</p>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={() => handleEdit(subscription)} className="p-2 rounded-lg text-slate-500 hover:text-cyan-700 hover:bg-cyan-100 transition-all" title="Editar">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                      </button>
                      <button onClick={() => handleDelete(subscription._id)} className="p-2 rounded-lg text-slate-500 hover:text-red-700 hover:bg-red-100 transition-all" title="Eliminar">
                ...
                {subscription.notes && (
                  <p className="text-xs text-slate-500 mt-3 pt-3 border-t border-slate-100">
                    <span className="text-slate-600">Notas:</span> {subscription.notes}
                  </p>
                )}
                <div className="sm:hidden mt-2 flex items-center gap-2">
                  <SABadge variant={getStatusVariant(subscription.status)} dot>{getStatusText(subscription.status)}</SABadge>
                  <span className="text-[11px] text-slate-500">Hasta {new Date(subscription.endDate).toLocaleDateString('es-ES')}</span>
                </div>
              </motion.div>
            );
          })}
      ...
      >
        <form id="sub-form" onSubmit={handleSubmit} className="space-y-5">
          {/* Business */}
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">Negocio <span className="text-red-600">*</span></label>
            ...
            {formErrors.businessId && <p className="text-red-600 text-xs mt-1">{formErrors.businessId}</p>}
          </div>

          {/* Commercial plan and cycle */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Plan Comercial <span className="text-red-600">*</span></label>
              ...
              {formErrors.commercialPlan && <p className="text-red-600 text-xs mt-1">{formErrors.commercialPlan}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Ciclo de Cobro <span className="text-red-600">*</span></label>
              <select
                name="billingCycle"
                value={formData.billingCycle}
                onChange={handleInputChange}
                disabled={formData.commercialPlan === 'free'}
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none focus:border-cyan-500 transition-all disabled:opacity-60 disabled:cursor-not-allowed [&>option]:bg-white"
                required
              >
              ...
              {formData.commercialPlan === 'free' && (
                <p className="text-[11px] text-slate-500 mt-1">Gratis siempre usa ciclo mensual.</p>
              )}
              {formErrors.billingCycle && <p className="text-red-600 text-xs mt-1">{formErrors.billingCycle}</p>}
            </div>
          </div>

          {/* Price */}
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">Precio <span className="text-red-600">*</span></label>
            <div className="relative">
              ...
              <input
                ...
                onChange={handleInputChange}
                className="w-full pl-7 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none focus:border-cyan-500 transition-all"
                placeholder="0"
                step="0.01"
                min="0"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1.5">Tipo de periodo (legacy)</label>
            <input
              type="text"
              value={formData.planType === 'annual' ? 'Anual' : 'Mensual'}
              readOnly
              className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-700 text-sm outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Fecha Inicio <span className="text-red-600">*</span></label>
              <input type="date" name="startDate" value={formData.startDate} onChange={handleInputChange}
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none focus:border-cyan-500 transition-all [color-scheme:dark]"
                required />
              {formErrors.startDate && <p className="text-red-600 text-xs mt-1">{formErrors.startDate}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Fecha Fin <span className="text-red-600">*</span></label>
              <input type="date" name="endDate" value={formData.endDate} onChange={handleInputChange}
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-none focus:border-cyan-500 transition-all [color-scheme:dark]"
                required />
              {formErrors.endDate && <p className="text-red-600 text-xs mt-1">{formErrors.endDate}</p>}
            </div>
          </div>

          {/* Duration info */}
          {formData.startDate && formData.endDate && (
            <div className="bg-cyan-500/[0.06] border border-cyan-200 rounded-lg px-3 py-2.5 flex items-center gap-2">
              <svg className="w-4 h-4 text-cyan-600 shrink-0" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
