...
/* ─── KPI Card ─── */
const ACCENTS = {
  blue:    { dot: 'bg-blue-500',    text: 'text-blue-600 dark:text-blue-400',       glow: 'shadow-blue-500/10 dark:shadow-blue-400/10' },
  cyan:    { dot: 'bg-cyan-500',    text: 'text-cyan-600 dark:text-cyan-400',       glow: 'shadow-cyan-500/10 dark:shadow-cyan-400/10' },
  emerald: { dot: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400', glow: 'shadow-emerald-500/10 dark:shadow-emerald-400/10' },
  violet:  { dot: 'bg-violet-500',  text: 'text-violet-600 dark:text-violet-400',   glow: 'shadow-violet-500/10 dark:shadow-violet-400/10' },
};
...
function KPICard({ label, value, sub, accent = 'blue', delay = 0, icon }) {
  ...
  return (
    <motion.div
      ...
      className="relative bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow"
    >
      {/* Accent line on left */}
      ...
      <div className="pl-2">
        <div className="flex items-center gap-1.5 mb-1.5">
          ...
          <span className="text-[10px] font-bold text-slate-500 dark:text-white/40 uppercase tracking-[0.08em]">{label}</span>
        </div>
        <p className={`text-[26px] sm:text-[28px] font-bold tabular-nums leading-none ${a.text}`}>{value}</p>
        {sub && <p className="text-[11px] text-slate-500 dark:text-white/40 mt-2 tabular-nums leading-relaxed">{sub}</p>}
      </div>
    </motion.div>
  );
}
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-[12px] font-bold text-slate-700 dark:text-white/70 uppercase tracking-[0.08em]">{title}</h2>
        {action}
      </div>
      {children}
    </section>
...
function AlertPill({ tone = 'amber', label, count, onClick }) {
  ...
  const toneMap = {
    amber: 'bg-amber-50 border-amber-300 text-amber-900 hover:bg-amber-100 dark:bg-amber-500/10 dark:border-amber-500/30 dark:text-amber-300 dark:hover:bg-amber-500/20',
    red: 'bg-red-50 border-red-300 text-red-900 hover:bg-red-100 dark:bg-red-500/10 dark:border-red-500/30 dark:text-red-300 dark:hover:bg-red-500/20',
    blue: 'bg-blue-50 border-blue-300 text-blue-900 hover:bg-blue-100 dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-300 dark:hover:bg-blue-500/20',
  };
  return (
    <button
      type="button"
    ...
    >
      <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-md bg-white/70 dark:bg-black/30 text-[11px] font-bold tabular-nums">
        {count}
      </span>
      <span>{label}</span>
...
export default function Home({ onNavigate, userName }) {
  ...
  if (loading) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-20 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl" />
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-28 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl" />
          ))}
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="h-72 bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      {/* ─── Hero greeting ─── */}
      <div className="relative overflow-hidden bg-gradient-to-br from-cyan-500/[0.08] via-blue-500/[0.05] to-violet-500/[0.06] dark:from-cyan-500/[0.12] dark:via-blue-500/[0.08] dark:to-violet-500/[0.1] border border-slate-200 dark:border-white/[0.08] rounded-xl p-5 sm:p-6">
        <div className="absolute -top-12 -right-12 w-40 h-40 bg-cyan-500/10 dark:bg-cyan-400/10 rounded-full blur-2xl pointer-events-none" />
        <div className="absolute -bottom-12 -left-12 w-40 h-40 bg-violet-500/10 dark:bg-violet-400/10 rounded-full blur-2xl pointer-events-none" />
        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <p className="text-[22px] sm:text-[26px] font-bold text-slate-900 dark:text-white leading-tight tracking-tight">
              {greet()}{userName ? `, ${userName.split(' ')[0]}` : ''} 👋
            </p>
            <p className="text-[13px] text-slate-600 dark:text-white/60 mt-1">
              Aquí está el pulso de MenuBy
              {lastUpdated && <span className="text-slate-400 dark:text-white/30"> · actualizado {lastUpdated}</span>}
            </p>
          </div>
          <button
            onClick={() => { setLoading(true); load(); }}
            className="text-xs font-semibold text-slate-700 dark:text-white/70 hover:text-slate-900 dark:hover:text-white px-3.5 py-2 rounded-lg bg-white/80 dark:bg-white/[0.06] border border-slate-200 dark:border-white/[0.1] hover:bg-white dark:hover:bg-white/[0.1] transition-all flex items-center gap-1.5 backdrop-blur-sm"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Section
          ...
          action={topBusinesses.length > 0 && (
            <button onClick={() => onNavigate?.('businesses')} className="text-[11px] font-semibold text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition flex items-center gap-1">
              Ver todos
        ...
        >
          <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.06] rounded-xl overflow-hidden shadow-sm">
            {topBusinesses.length === 0 ? (
              ...
              topBusinesses.map((b, i) => {
                ...
                return (
                  <div key={b.businessId || i} className={`px-4 py-3 ${i < topBusinesses.length - 1 ? 'border-b border-slate-100 dark:border-white/[0.04]' : ''}`}>
                    <div className="flex items-center gap-3">
                      <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-cyan-500/20 to-blue-500/20 dark:from-cyan-400/20 dark:to-blue-400/20 flex items-center justify-center text-[11px] font-bold text-cyan-700 dark:text-cyan-300 shrink-0">
                        {i + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{b.name}</p>
                        <p className="text-[11px] text-slate-500 dark:text-white/40 font-mono truncate">{b.slug || '—'}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-sm font-bold tabular-nums text-cyan-600 dark:text-cyan-400">{formatNumber(b.count)}</p>
                        <p className="text-[11px] tabular-nums text-slate-500 dark:text-white/40">{formatShortCOP(b.gmv)}</p>
                      </div>
                    </div>
                    {/* Bar */}
                    <div className="mt-2 h-1 bg-slate-100 dark:bg-white/[0.05] rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${pct}%` }}
        ...
        <Section
          ...
          action={recent.length > 0 && (
            <button onClick={() => onNavigate?.('audit')} className="text-[11px] font-semibold text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition flex items-center gap-1">
              Ver log
              <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
            </button>
          )}
                ...
                return (
                  <div key={log._id || i} className={`px-4 py-3 ${i < recent.length - 1 ? 'border-b border-slate-100 dark:border-white/[0.04]' : ''}`}>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[11px] text-slate-500 dark:text-white/35 shrink-0">{formatRelative(log.createdAt)}</span>
                    </div>
